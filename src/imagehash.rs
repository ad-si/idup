...
impl Ord for ImageHashAV {
  fn cmp(&self, other: &Self) -> Ordering {
    ...
    if self.image_path.always_mark_dupe_compare
      && self.image_path.is_compare_dir
      && (!other.image_path.always_mark_dupe_compare)
    {
      return Ordering::Greater;
    }
    ...
    if (!self.image_path.always_mark_dupe_compare)
      && other.image_path.always_mark_dupe_compare
      && other.image_path.is_compare_dir
    {
      return Ordering::Less;
    }
...
fn load_image_from_file(
  image_path: &str,
) -> std::result::Result<DynamicImage, MyImageError> {
  let img = match ImageReader::open(image_path) {
    Ok(image) => image,
    Err(_) => {
      return Err(MyImageError::FileError(
        format!("Error: Failed to read image file: {}", image_path).to_string(),
      ));
    }
  };
  ...
  let format_guessed = match img.with_guessed_format() {
    ...
    Err(_) => {
      return Err(MyImageError::DecodeFail(
        format!("Error: Failed to identify image file format {}", image_path)
          .to_string(),
      ));
    }
  };
  ...
  let decoded_img = match format_guessed.decode() {
    ...
    Err(_) => {
      return Err(MyImageError::DecodeFail(
        format!("Error: Failed to correctly decode image: {}", image_path)
          .to_string(),
      ));
    }
  };
...
impl ImageHashAV {
  ...
  pub fn new(
    ...
    min_width: u32,
    min_height: u32,
  ) -> Result<ImageHashAV, MyImageError> {
    let mut object = ImageHashAV {
      dupe_group: 0,
      grey_hash: 0,
      low_res: [0; 192],
      width: 0,
  ...
  pub fn has_similar_aspect_ratio(&self, comp: &ImageHashAV) -> bool {
    ...
    if aspect_ratio_b <= aspect_ratio_a_high
      && aspect_ratio_b >= aspect_ratio_a_low
    {
      return true;
    }
  ...
  pub fn diff_colour(&self, comp: &ImageHashAV) -> u64 {
    ...
    for i in 0..64 {
      let rdiff: u32 = (comp.low_res[i * 3] as i32
        - self.low_res[i * 3] as i32)
        .unsigned_abs();
      let gdiff: u32 = (comp.low_res[(i * 3) + 1] as i32
        - self.low_res[(i * 3) + 1] as i32)
        .unsigned_abs();
      let bdiff: u32 = (comp.low_res[(i * 3) + 2] as i32
        - self.low_res[(i * 3) + 2] as i32)
        .unsigned_abs();
  ...
  pub fn is_dupe(&self, other: &ImageHashAV, config: &ConfigOptions) -> bool {
    ...
    if self.std_dev > config.std_dev_threshold
      && other.std_dev > config.std_dev_threshold
    {
  ...
  pub fn calc_std_dev_colour_hash(&mut self) {
    ...
    for i in 0..64 {
      ...
      g_square_total +=
        ((self.low_res[(i * 3) + 1] as f32) - g_pixel_av).powf(2f32);
      b_square_total +=
        ((self.low_res[(i * 3) + 2] as f32) - b_pixel_av).powf(2f32);
    }
    r_square_total /= 64.0;
    g_square_total /= 64.0;
    b_square_total /= 64.0;
    ...
    self.std_dev =
      (r_square_total.sqrt() + g_square_total.sqrt() + b_square_total.sqrt())
        / 3.0;
  }

  /// Populates image statistics including the perceptual hash
  pub fn calc_image_hash(
    &mut self,
    im_path: &ImagePath,
    min_width: u32,
    min_height: u32,
    ...
    match load_image_from_file(&im_path.fpath) {
      Ok(img) => {
        ...
        if width < 16 || height < 16 {
          return Some(MyImageError::ImageTooSmall(
            format!(
              "Warning: Image too small to deduplicate: {}",
              im_path.fpath
            )
            .to_string(),
          ));
        ...
        match fs::metadata(im_path.fpath.clone()) {
          ...
          Err(_) => {
            return Some(MyImageError::FileError(
              format!("Error: Failed to get size of: {}", im_path.fpath)
                .to_string(),
            ));
          }
        }
        ...
        if width != 8 || height != 8 {
          return Some(MyImageError::DecodeFail(
            format!(
              "Error: Failed to resize image correctly: {}",
              im_path.fpath
            )
            .to_string(),
          ));
...
mod tests {
  ...
  #[test]
  fn test_image_read() {
    let result = ImageHashAV::new(
      &ImagePath {
        fpath: "unit_test_images/bridge1_best.jpg".to_string(),
        is_compare_dir: false,
        always_mark_dupe_compare: false,
      },
      0,
  ...
  fn test_image_duplicates() {
    ...
    for entry in
      glob("unit_test_images/*").expect("Failed to read glob pattern")
    {
    ...
    assert!(
      image_paths.len() % 3 == 0,
      "Image paths divides by 3 (best + 2 duplicates per image)"
    );
    ...
    for i in 0..(image_paths.len() / 3) {
      ...
      .unwrap();
      let dupe1 = ImageHashAV::new(
        &ImagePath {
          fpath: image_paths[(i * 3) + 1].clone(),
          is_compare_dir: false,
          always_mark_dupe_compare: false,
        },
        0,
      ...
      .unwrap();
      let dupe2 = ImageHashAV::new(
        &ImagePath {
          fpath: image_paths[(i * 3) + 2].clone(),
          is_compare_dir: false,
          always_mark_dupe_compare: false,
        },
        0,
      ...
      assert!(
        calc_hamming_distance(result.dupe_group, dupe1.dupe_group) >= 63,
        "First duplicate grey hash matches"
      );
      assert!(
        calc_hamming_distance(result.dupe_group, dupe2.dupe_group) >= 63,
        "Second duplicate grey hash matches"
      );

      assert!(
        result.diff_colour(&dupe1)
          <= ImageHashAV::DEFAULT_COLOUR_DIFF_THRESHOLD,
        "First duplicate colours are similar"
      );
      assert!(
        result.diff_colour(&dupe2)
          <= ImageHashAV::DEFAULT_COLOUR_DIFF_THRESHOLD,
        "Second duplicate colours are similar"
      );
    }
  }
      ...
      for j in (i + 1)..image_hashes.len() {
        ...
        assert!(
          (calc_hamming_distance(
            image_hashes[j].dupe_group,
            image_hashes[i].dupe_group
          ) <= 63)
            || (image_hashes[j].diff_colour(&image_hashes[i])
              > ImageHashAV::DEFAULT_COLOUR_DIFF_THRESHOLD),
